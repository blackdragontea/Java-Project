...
namespace QuanLyBanHang.banhang
{
    public partial class banhang : Form
    {
        ...
        public  void Load_dta1()
        {
            ...
            db1.Columns[0].HeaderText = "Mã sản phẩm";
            db1.Columns[1].HeaderText = "Tên sản phẩm";
            db1.Columns[2].HeaderText = "Đơn vị tính";
            db1.Columns[3].HeaderText = "Số lượng";
            db1.Columns[4].HeaderText = "Giá bán";
        }
        ...
        private void dta2_Click(object sender, EventArgs e)
        {
            try
            {
                // Kiểm tra nếu dta2 không null và có ít nhất một hàng được chọn
                if (dta2 != null && dta2.CurrentRow != null && dta2.CurrentRow.Index >= 0 && dta2.CurrentRow.Index < dta2.Rows.Count)
                {
                    // Vô hiệu hóa nút thêm và textbox số lượng bán, kích hoạt nút xóa
                    btn_Add.Enabled = false;
                    txt_slgban.Enabled = false;
                    btn_Xoa.Enabled = true;

                    // Lấy chỉ số của hàng hiện tại trong DataGridView
                    int curow = dta2.CurrentRow.Index;

                    // Kiểm tra lại dữ liệu trong ô trước khi truy cập
                    if (curow >= 0 && curow < dta2.Rows.Count)
                    {
                        // Lấy dữ liệu từ các ô của hàng hiện tại và gán vào các textbox
                        ...
                        // Truy vấn để lấy số lượng từ cơ sở dữ liệu
                        string solgQuery = "SELECT solg FROM sanpham WHERE masp ='" + txt_mathuoc.Text + "'";
                        SqlDataReader rd3 = chuoiketnoi.showtext(solgQuery);

                        // Đảm bảo chỉ đọc dữ liệu khi có dữ liệu để đọc
                        if (rd3.Read())
                        {
                            a = int.Parse(rd3[0].ToString());
                        }
                        rd3.Close(); // Đóng DataReader sau khi sử dụng để giải phóng tài nguyên
                    }
                    else
                    {
                        MessageBox.Show("Hàng hiện tại không có dữ liệu.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
